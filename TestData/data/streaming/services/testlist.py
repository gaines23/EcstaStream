...
class Provider(TMDb):
    ...
    #     return self._get_obj(self._call(self._urls["tv_provider"], urlencode(params)))
...
#shows = provider.tv_provider({"watch_region":"US"})

providerFile = open("data/provider/provider.json", "w", encoding='utf-8')
...
for m in movies:
    ...
    #providerFile.write('{"display_priority":%d}\n' %(m.display_priority))
